...
- 用 `tasks / runtime / sessions / knowledge / deliverables / learning` 保存项目真相
...
如果在执行过程中纠正了 workflow 错误、修复了重复问题，或用户明确指出了 agent 的失误，建议同步记录到学习系统：
...
webforge learn lesson "经验标题" \
  --content "简要教训" \
  --priority high
```

这样下一次 `resume` 或显式执行 `webforge learn remind --task <task-id>` 时，就可以优先看到相关提醒。
